<?php
 class Libvirt {
  ...
  function Libvirt($debug = false) {
   if ($debug) $this->set_logfile($debug);
  }
  ...
  function set_logfile($filename) {
   if (!libvirt_logfile_set($filename)) return $this->_set_last_error();
   return true;
  }
  ...
  function generate_uuid() {
   $ret = array();
   for ($i = 0; $i < 16; $i++) $ret[] = $this->macbyte(rand() % 256);
  ...
  function storagevolume_get_xml_desc($pool, $volume) {
   $res_pool = libvirt_storagepool_lookup_by_name($this->conn, $pool);
   $res_vol = libvirt_storagevolume_lookup_by_name($res_pool, $volume);
   return libvirt_storagevolume_get_xml_desc($res_vol, NULL);
  }
  ...
  function storagepool_undefine($pool) {
   $res = libvirt_storagepool_lookup_by_name($this->conn, $pool);
   if (libvirt_storagepool_is_active($res)) libvirt_storagepool_destroy($res);
   return libvirt_storagepool_undefine($res);
  }

  function storagepool_lookup_by_name($name) {
   return libvirt_storagepool_lookup_by_name($this->conn, $name);
  }

  function storagepool_get_xml_desc($res) {
   return libvirt_storagepool_get_xml_desc($res);
  }

  function get_iso_images($path) {
   return libvirt_get_iso_images($path);
  }

  function domain_get_disk_devices($domain) {
  ...
  function domain_generate_uuid() {
   ...
   while (@libvirt_domain_lookup_by_uuid_string($this->conn, $uuid)) $uuid = $this->generate_uuid();
   return $uuid;
  }
  ...
  function domain_create($name, $ram, $cpu, $iso, $disks, $nets) {
  ...
  function domain_get_screenshot($domain, $convert = 1) {
   ...
   if ($tmp == false) return $this->_set_last_error();
   $mime = $tmp['mime'];
  ...
  function domain_get_screen_dimensions($domain) {
   ...
   if (file_exists($imgFile) && $screen) list($width, $height) = getimagesize($imgFile);
   unlink($imgFile);
  ...
  function generate_random_mac_addr($seed = false) {
   if (!$seed) $seed = 1;
   if ($this->get_hypervisor_name() == 'qemu') $prefix = '52:54:00';
   else {
    if ($this->get_hypervisor_name() == 'xen') $prefix = '00:16:3e';
    else $prefix = $this->macbyte(($seed * rand()) % 256).':' . $this->macbyte(($seed * rand()) % 256) . ':' . $this->macbyte(($seed * rand()) % 256);
   }
   return $prefix . ':' . $this->macbyte(($seed * rand()) % 256) . ':' . $this->macbyte(($seed * rand()) % 256) . ':' . $this->macbyte(($seed * rand()) % 256);
  }

  function domain_nic_add($domain, $mac, $network, $model=false) {
   $dom = $this->get_domain_object($domain);
   if ($model == 'default') $model = false;
  ...
  function get_xpath($domain, $xpath, $inactive = false) {
   $dom = $this->get_domain_object($domain);
   $flags = 0;
   if ($inactive) $flags = VIR_DOMAIN_XML_INACTIVE;
  ...
  function get_network_cards($domain) {
   ...
   if (!is_array($nics)) return $this->_set_last_error();
   return $nics['num'];
  }
  ...
  function get_disk_capacity($domain, $physical=false, $disk='*', $unit='?') {
   ...
   for ($i = 0; $i < sizeof($tmp); $i++) {
    if (($disk == '*') || ($tmp[$i]['device'] == $disk))
     if ($physical) $ret += $tmp[$i]['physical'];
     else $ret += $tmp[$i]['capacity'];
   }
   unset($tmp);
  ...
  function get_storagepool_info($name) {
   ...
   $tmp['id_user'] = $otmp1;
   $tmp['id_group'] = $otmp2;
   if ($tmp['active']) $tmp['volume_count'] = sizeof(libvirt_storagepool_list_volumes($res));
   else $tmp['volume_count'] = 0;
   return $tmp;
  }

  function storagepool_get_volume_information($pool, $name=false) {
   if (!is_resource($pool)) $pool = $this->get_storagepool_res($pool);
  ...
  function storagevolume_delete($pool, $volume) {
   $res_pool = libvirt_storagepool_lookup_by_name($this->conn, $pool);
   $res_vol = libvirt_storagevolume_lookup_by_name($res_pool, $volume);
   if ($res_vol) {
    if (libvirt_storagevolume_delete($res_vol)) return true;
    else return false;
   } else return false;
  }
  ...
  function storagevolume_create($pool, $name, $format, $capacity, $capacity_unit, $allocation, $allocation_unit) {
   $pool = $this->get_storagepool_res($pool);
   $capacity = $this->parse_size($capacity);
   $allocation = $this->parse_size($allocation);
   $xml = '<volume>' . "\n" .
          ' <name>' . $name . '</name>' . "\n" .
          ' <allocation unit="' . $allocation_unit . '">' . $allocation . '</allocation>' . "\n" .
          ' <capacity unit="' . $capacity_unit . '">' . $capacity . '</capacity>' . "\n" .
          ' <target>' . "\n" .
          '  <format type="' . $format . '"/>' . "\n" .
          ' </target>' . "\n" .
          '</volume>';
  ...
  function domain_change_xml($domain, $xml) {
   $dom = $this->get_domain_object($domain);
   if (!($old_xml = libvirt_domain_get_xml_desc($dom, NULL))) return $this->_set_last_error();
   if (!libvirt_domain_undefine($dom)) return $this->_set_last_error();
  ...
  function network_change_xml($network, $xml) {
   ...
   if (!($old_xml = libvirt_network_get_xml_desc($net, NULL))) return $this->_set_last_error();
   if (!libvirt_network_undefine($net)) return $this->_set_last_error();
  ...
  function get_network_res($network) {
   ...
   if (is_resource($network)) return $network;
  ...
  function set_network_active($network, $active = true) {
   $res = $this->get_network_res($network);
   if ($res == false) return false;
   if (!libvirt_network_set_active($res, $active ? 1 : 0)) return $this->_set_last_error();
   return true;
  }
  ...
  function get_node_device_cap_options() {
   ...
   for ($i = 0; $i < sizeof($all); $i++) {
    ...
    for ($ii = 0; $ii < sizeof($tmp); $ii++) if (!in_array($tmp[$ii], $ret))$ret[] = $tmp[$ii];
   }
   return $ret;
  }
  ...
  function domain_get_info($dom) {
   if (!$this->allow_cached) return libvirt_domain_get_info($dom);
  ...
  function domain_get_boot_devices($domain) {
   ...
   if (!$tmp) return false;
   $devs = array();
   for ($i = 0; $i < $tmp['num']; $i++) $devs[] = $tmp[$i];
   return $devs;
  }
  ...
  function domain_get_multimedia_device($domain, $type, $display=false) {
   ...
   if ($type == 'console') {
    ...
    if ($display) return $type.' ('.$targetType.' on port '.$targetPort.')';
    else return array('type' => $type, 'targetType' => $targetType, 'targetPort' => $targetPort);
   } else if ($type == 'input') {
    ...
    if ($display) return $type.' on '.$bus;
    else return array('type' => $type, 'bus' => $bus);
    ...
    if ($display) return $type.' on port '.$port.' with'.($autoport ? '' : 'out').' autoport enabled';
    else return array('type' => $type, 'port' => $port, 'autoport' => $autoport);
   } else if ($type == 'video') {
    ...
    if ($display) return $type.' with '.($vram / 1024).' MB VRAM, '.$heads.' head(s)';
    else return array('type' => $type, 'vram' => $vram, 'heads' => $heads);
   } else return false;
  }
  ...
  function domain_set_feature($domain, $feature, $val) {
   ...
   if ($val) {
    if (strpos('features', $xml)) $xml = str_replace('<features>', "<features>\n<$feature/>", $xml);
    else $xml = str_replace('</os>', "</os><features>\n<$feature/></features>", $xml);
   } else $xml = str_replace("<$feature/>\n", '', $xml);
  ...
  function domain_set_description($domain, $desc) {
   ...
   $xml = $this->domain_get_xml($domain, true);
   if (!$description) $xml = str_replace("</uuid>", "</uuid><description>$desc</description>", $xml);
   else {
    $tmp = explode("\n", $xml);
    for ($i = 0; $i < sizeof($tmp); $i++) if (strpos('.'.$tmp[$i], '<description')) $tmp[$i] = "<description>$desc</description>";
    $xml = join("\n", $tmp);
   }
   return $this->domain_change_xml($domain, $xml);
  }
  ...
  function get_nwfilter_xml($uuid) {
   ...
   if (!$nwfilter) return $this->_set_last_error();
